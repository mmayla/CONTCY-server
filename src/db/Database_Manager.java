...
public class Database_Manager
{
	...
	public String GetBody(String url)
	{
		...
		{
			ResultSet rs1;
			int Uid = Get_ID_URL(url);
			if (Uid != -1)
			{
				stmt = connection.createStatement();
				rs1 = stmt
						.executeQuery("select * from Crawler_Database.content where C_ID ="
								+ Uid + " ;");
				if (rs1.next())
				{
					body_url = rs1.getString("T_Page");
				} else
				{
					System.out.println("Can not found the body of URL");
				}

			} else
			{
				System.out.println("Can not found the URL");
			}
	...
	// ===================================================
	public int Get_ID_URL(String url)
	{
		int Uid = -1;
		try
		{
			...
			stmt = connection.createStatement();
			rs1 = stmt
					.executeQuery("select U_ID from Crawler_Database.URLs where URL ='"
							+ url + "' ;");
			if (rs1.next())
			{
				Uid = rs1.getInt("U_ID");
	...
	public Boolean Get_Statement(String url, String Statement)
	{
		int uid = Get_ID_URL(url);
		if (uid == -1)
			return false;
